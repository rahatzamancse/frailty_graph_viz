...
def resolve_document(seen_in: str, bibilography:Mapping[str, XDDMetaData]) -> str:
...
def main(index_factors_path: Optional[Path] = None,
         ...
         *input_files_dirs#=['/home/enrique/data/arizona_associations_markup/', '/media/evo870/data/xDD_extractions/arizona_files/']
         ):
    """
    Reads
    """
    ...
    for row in tqdm(all_rows, desc='Fixing participant\'s names'):
        try:
            ...
            if row['EVENT LABEL'] == 'Significance':
                ...
                value = row['OUTPUT']
                paper = resolve_document(row['SEEN IN'], xdd_bib)
            ...
            try:
                ...
                if len(controllers) > 0:
                    for controller, input, output in it.product(controllers, inputs, outputs):
                        ...
                        output = output[1]
                        freq = row['SEEN']
                        doc = resolve_document(row['SEEN IN'], xdd_bib)
